...
<body>
    ...
    <div style="height: 452px; margin-top: 0px" align="center">
               <asp:SqlDataSource ID="SqlDataSourceShipment" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>" SelectCommand="SELECT * FROM [SHIPMENT]" DeleteCommand="DELETE FROM [SHIPMENT] WHERE [Shipment_Id] = @Shipment_Id" InsertCommand="INSERT INTO [SHIPMENT] ([Shipment_Date], [Sorder_Number], [Inventory_Id], [Customer_Id], [Due_Date]) VALUES (@Shipment_Date, @Sorder_Number, @Inventory_Id, @Customer_Id, @Due_Date)" UpdateCommand="UPDATE [SHIPMENT] SET [Shipment_Date] = @Shipment_Date, [Sorder_Number] = @Sorder_Number, [Inventory_Id] = @Inventory_Id, [Customer_Id] = @Customer_Id, [Due_Date] = @Due_Date WHERE [Shipment_Id] = @Shipment_Id">
            <DeleteParameters>
